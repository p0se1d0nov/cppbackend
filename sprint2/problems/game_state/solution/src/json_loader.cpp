...
namespace json_loader
{
    ...
    model::Game LoadGame(const std::filesystem::path &json_path)
    {
        ...
        if (value.as_object().contains("players"))
        {
            const auto &players_array = value.as_object().at("players").as_array();
            for (const auto &player_val : players_array)
            {
                const auto &player_obj = player_val.as_object();
                std::string player_id = json::value_to<std::string>(player_obj.at("id"));
                std::string token = json::value_to<std::string>(player_obj.at("token"));
                std::string map_id;
                if (player_obj.contains("mapId"))
                {
                    map_id = json::value_to<std::string>(player_obj.at("mapId"));
                }

                const model::Map *map = nullptr;
                if (!map_id.empty())
                {
                    map = game.FindMap(model::Map::Id{map_id});
                }
                if (!map && !game.GetMaps().empty())
                {
                    map = &game.GetMaps().front();
                }

                model::PointF position{0.0, 0.0};
                if (map && !map->GetRoads().empty())
                {
                    const auto &roads = map->GetRoads();
                    const auto hash = std::hash<std::string>{}(token);
                    const size_t road_index = hash % roads.size();
                    const auto &road = roads[road_index];
                    const double t = static_cast<double>((hash >> 32) & 0xFFFFFFFFu) / static_cast<double>(0xFFFFFFFFu);
