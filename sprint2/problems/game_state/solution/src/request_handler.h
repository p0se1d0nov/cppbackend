...
namespace http_handler
{
    ...
    public:
        ...
        {
            auto make_json_response = [&req](http::status status, std::string body, bool head = false)
            {
            ...
            {
                // API handling
                const bool allow_get_head = req.method() == http::verb::get || req.method() == http::verb::head;

                if (path_str == "/api/v1/maps" && allow_get_head)
                {
                    ...
                    auto res = make_json_response(http::status::ok, json::serialize(maps_json), req.method() == http::verb::head);
                    send(std::move(res));
                    return;
                }

                if (path_str.rfind("/api/v1/maps/", 0) == 0 && allow_get_head && path_str.size() > 13)
                {
                    ...
                    if (const model::Map *map = game_.FindMap(model::Map::Id{map_id}))
                    {
                        ...
                        auto res = make_json_response(http::status::ok, json::serialize(map_json), req.method() == http::verb::head);
                        send(std::move(res));
                        return;
                    }
                    else
                    {
                        ...
                        auto res = make_json_response(http::status::not_found, json::serialize(error_json), req.method() == http::verb::head);
                        send(std::move(res));
                        return;
                    }
                }

                if (path_str == "/api/v1/game/state")
                {
                    if (!allow_get_head)
                    {
                        auto error_json = ErrorResponse("invalidMethod", "Invalid method");
                    ...
                    const std::string auth_value{auth_header};
                    if (auth_value.size() <= bearer_prefix.size() || auth_value.substr(0, bearer_prefix.size()) != bearer_prefix)
                    {
                        auto error_json = ErrorResponse("invalidToken", "Authorization header is required");
                        auto res = make_json_response(http::status::unauthorized, json::serialize(error_json), req.method() == http::verb::head);
                        send(std::move(res));
                        return;
                    }

                    const std::string token = auth_value.substr(bearer_prefix.size());
                    ...
                    if (game_.FindPlayerByToken(token) == nullptr)
                    {
                        auto error_json = ErrorResponse("unknownToken", "Player token has not been found");
                        auto res = make_json_response(http::status::unauthorized, json::serialize(error_json), req.method() == http::verb::head);
                        send(std::move(res));
                        return;
                    }
                    ...
                    for (const auto &player : game_.GetPlayers())
                    {
                        ...
                        player_json["pos"] = json::array{player.GetPosition().x, player.GetPosition().y};
                        player_json["speed"] = json::array{player.GetSpeed().dx, player.GetSpeed().dy};
                        ...
                        player_json["dir"] = std::string(1, dir_symbol);
                        players_json[*player.GetId()] = player_json;
                    }
                    state_json["players"] = players_json;

                    auto res = make_json_response(http::status::ok, json::serialize(state_json), req.method() == http::verb::head);
                    send(std::move(res));
                    return;
                }
