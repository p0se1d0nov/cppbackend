    ...
    for (const auto& event: events) {
        ...
        if (event.item_id < loot_.size()) {      // обрабатываем предмет
            ...
            if (!dog->IsBagFull(map_.GetBagCapacity()) && loot.on_map ) {
                ...
                std::cout << "Fuck FuckFuckFuckFuckFuckFuckFuckFuckFuckFuckFuckFuck" << std::endl << std::endl << std::endl;
            }
        }
        else {
            dog->ClearBag();
...
Item ModelCollisionProvider::GetItem(size_t idx) const {
    if (idx < loot_.size()) {
...
Gatherer ModelCollisionProvider::GetGatherer(size_t idx) const {
