...
bool View::AddBook(std::istream& cmd_input) const {
    try {
        if (auto params = GetBookParams(cmd_input)) {
            use_cases_.AddBook(params->publication_year,params->title,domain::AuthorId::FromString(params->author_id));
...
std::vector<detail::BookInfo> View::GetBooks() const {
    ...
    for (const auto& book : all_books) {
        books.push_back({book.GetTitle(), book.GetYear()});
    }
    return books;
}
...
std::vector<detail::BookInfo> View::GetAuthorBooks(const std::string& author_id) const {
    std::vector<detail::BookInfo> books;
    auto author_books = use_cases_.ShowAuthorsBook(domain::AuthorId::FromString(author_id));
