...
public:
    ...
    void operator()(http::request<Body, http::basic_fields<Allocator>>&& req, Send&& send) {
        try {
            ...
            if (target.starts_with(endpoints_.ApiPrefix())) {
...
private:
    ...
    template <typename Body, typename Allocator, typename Send>
    void HandleRecordRequest(http::request<Body, http::basic_fields<Allocator>>&& req, Send&& send);

    http_endpoints::HttpEndpoints endpoints_;
    fs::path static_dir_;
    Strand api_strand_;
...
template <typename Body, typename Allocator, typename Send>
void RequestHandler::HandleRecordRequest(http::request<Body, http::basic_fields<Allocator>>&& req, Send&& send){
    boost::urls::url_view url(req.target());
    auto params = url.params();
    std::optional<std::string_view> start_sv;
    std::optional<std::string_view> max_sv;
    ...
    try {
        for (auto [key, value, has_value] : params) {
            if (key == "start" && has_value) {
                start = std::stoull(value);
            }
            if (key == "maxItems" && has_value) {
                maxItems =std::stoull(value);
            }
        }
    ...
    if (maxItems > 100) {
        return MakeErrorResponse(std::forward<Send>(send),
                                 http::status::bad_request,
                                 "invalidArgument",
                                 "maxItems more than 100",
                                 req.version());
    }

    try {
        auto records = app_.GetRecords(start,maxItems);
        ...
        std::string body = boost::json::serialize(result);
...
void RequestHandler::HandleApiRequest(http::request<Body, http::basic_fields<Allocator>>&& req, Send&& send) {
    std::string_view target = req.target();
    const std::string_view V1_PREFIX = endpoints_.ApiV1Prefix();
    ...
    if (target.starts_with(endpoints_.MapById())) {
        HandleMapRequest(std::move(req), std::forward<Send>(send));
        return;
    }

    if (target == endpoints_.GameJoin()) {
    ...
    } else if (target == endpoints_.GamePlayers()) {
    ...
    } else if (target == endpoints_.GameState()) {
        if (req.method() != http::verb::get && req.method() != http::verb::head) {
            return SendMethodNotAllowed(std::forward<Send>(send), "GET, HEAD",
                                        "Invalid method", req.version());
        }
        HandleGameStateRequest(std::move(req), std::forward<Send>(send));
    } else if (target == endpoints_.GamePlayerAction()) {
        if (req.method() != http::verb::post) {
            return SendMethodNotAllowed(std::forward<Send>(send), "POST",
                                        "Invalid method", req.version());
        }
        HandlePlayerActionRequest(std::move(req), std::forward<Send>(send));
    } else if (target == endpoints_.GameTick()) {
        if (req.method() != http::verb::post) {
            return SendMethodNotAllowed(std::forward<Send>(send), "POST",
                                        "Invalid method", req.version());
        }
        HandleGameTick(std::move(req), std::forward<Send>(send));
    } else if (target.starts_with(endpoints_.GameRecords())) {
        if (req.method() != http::verb::get) {
            return SendMethodNotAllowed(std::forward<Send>(send), "GET",
                                        "Invalid method", req.version());
        }
        HandleRecordRequest(std::move(req), std::forward<Send>(send));
    }
    else {
        MakeErrorResponse(std::forward<Send>(send),
                          http::status::bad_request,
...
void RequestHandler::HandleMapRequest(http::request<Body, http::basic_fields<Allocator>>&& req, Send&& send) {
    ...
    if (!target.starts_with(endpoints_.Maps())) {
        return MakeErrorResponse(std::forward<Send>(send),
                                 http::status::bad_request,
                                 "badRequest",
