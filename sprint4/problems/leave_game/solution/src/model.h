...
const int COEF_SEC_MILLISEC = 1000;
const double COEF_MILLISEC_SEC = 0.001;
...
public:
    ...
    void AddRoad(const Road& road) {
        ...
        if (road.IsHorizontal()) {
            horizontal_roads[road.GetStart().y].push_back(index);
        }
        else {
            vertical_roads[road.GetStart().x].push_back(index);
        }
    }
    ...
    const loot_gen::LootGenerator& GetLootGenerator() const {
        if (!generator_) {
            throw std::runtime_error("LootGenerator not set");
        }
        return *generator_;
    }
...
public:
    ...
    void ResetAFKTime() {dogAFKTime_ = 0;}
    void UpdateAFKTime(double time){ dogAFKTime_ += time;}
    void UpdateActiveTime(double time){ dogActiveTime_ += time;}
    double GetAFKTime() const { return dogAFKTime_;}
    double GetActiveTime() const { return dogActiveTime_ * COEF_MILLISEC_SEC;}
...
private:
    ...
    double dogAFKTime_ = 0;
    double dogActiveTime_ = 0;
    bool deleted_ = false;
};
...
class GameSession {
public:
    explicit GameSession(const Map& map, bool random_spawn,double dogRetirementTime)
        : map_(map)
...
public:
    ...
    void SetDogRetirementTime(double time) {dogRetirementTime_ = time * COEF_SEC_MILLISEC;}
    double GetDogRetirementTime() const {return dogRetirementTime_;}

private:
    using MapIdHasher = util::TaggedHasher<Map::Id>;
    using MapIdToIndex = std::unordered_map<Map::Id, size_t, MapIdHasher>;

    double dogRetirementTime_ = 60000; // В миллисекундах
