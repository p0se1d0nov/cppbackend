...
inline void ReportError(beast::error_code ec, std::string_view what) {
    logging::LogError(ec.value(), ec.message(), std::string(what));
}
...
class Session : public SessionBase, public std::enable_shared_from_this<Session<RequestHandler>> {
 ...
 private:
    void HandleRequest(HttpRequest&& request) override {
        ...
        logging::LogRequest(ip, std::string(request.target()), std::string(request.method_string()));

        auto send_wrapper = [self = this->shared_from_this(), start_time](auto&& response) {
            ...
            logging::LogResponse(start_time, status, content_type);
            self->Write(std::move(response));
        };

        request_handler_(std::move(request), std::move(send_wrapper));
    }
