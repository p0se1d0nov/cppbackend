...
int main(int argc, const char* argv[]) {
    try {
        ...
        const char* db_url = std::getenv("GAME_DB_URL");  // используем правильное имя переменной
        if (db_url) {
            pool = std::make_unique<ConnectionPool>(
                8,  // количество соединений
        ...
        app::Application app(game, players, db.get());   // создаём Application
