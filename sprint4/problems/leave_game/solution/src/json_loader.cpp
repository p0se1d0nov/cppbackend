...
void LoadRoads(model::Map& map, const boost::json::object& map_obj) {
    for (auto& road_val : map_obj.at("roads").as_array()) {
        ...
        if (auto x1_ptr = road_obj.if_contains("x1")) {
            int x0 = road_obj.at("x0").as_int64();
            int y0 = road_obj.at("y0").as_int64();
            int x1 = x1_ptr->as_int64();
...
model::Map LoadMap(const boost::json::object& map_obj,
    ...
    if (auto bag_cap_ptr = map_obj.if_contains("defaultBagCapacity")) {
        int cap = bag_cap_ptr->get_uint64();
...
model::Game LoadGame(const std::filesystem::path& json_path, bool random_spawn) {
    ...
    if (auto speed_ptr = config.as_object().if_contains("defaultDogSpeed")) {
         game.SetDefaultDogSpeed(speed_ptr->as_double());
    }

    double dogRetirementTime = 60;
    if (auto retirement_ptr = config.as_object().if_contains("dogRetirementTime")) {
        game.SetDogRetirementTime(retirement_ptr->as_double());
    }else {
    ...
    if (auto loot_config_ptr = config.as_object().if_contains("lootGeneratorConfig")) {
        auto& loot_cfg = loot_config_ptr->as_object();
